...
class _ConversationScreenState extends State<ConversationScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        appBar: AppBar(backgroundColor: Colors.white, elevation: 0, titleSpacing: 0, leading: BackButton(onPressed: () => context.canPop() ? context.pop() : context.go('/chats')), title: const Row(children: [CircleAvatar(radius: 21, backgroundColor: Color(0xFF37474F), child: Icon(Icons.person, color: Colors.white)), SizedBox(width: 10), Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Vikram Mehta', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: authInk)), Text('●  Online', style: TextStyle(fontSize: 10, color: Color(0xFF697187)))])]), actions: [IconButton(onPressed: () => context.push('/call/audio/${widget.conversationId}'), icon: const Icon(Icons.call, color: authPurple)), IconButton(onPressed: () => context.push('/call/video/${widget.conversationId}'), icon: const Icon(Icons.videocam, color: authPurple)), const Icon(Icons.more_vert, color: authInk), const SizedBox(width: 8)]),
