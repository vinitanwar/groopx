...
class GroupInfoScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const members = ['Rohan', 'Ananya Sharma', 'Vikram Mehta', 'Arjun', 'Neha Verma', 'Priya Singh', 'Karan Singh', 'Mehul Shah'];
    return Scaffold(backgroundColor: Colors.white, appBar: AppBar(backgroundColor: Colors.white, title: const Text('Group Info', style: TextStyle(fontWeight: FontWeight.w700)), actions: [TextButton(onPressed: () {}, child: const Text('Edit'))]), body: ListView(padding: const EdgeInsets.all(16), children: [
      ...
      ...members.indexed.map((entry) => ListTile(contentPadding: EdgeInsets.zero, leading: CircleAvatar(backgroundColor: entry.$1 < 5 ? const Color(0xFF263238) : authPurple, child: Text(entry.$2.substring(0, 1), style: const TextStyle(color: Colors.white))), title: Text(entry.$2, style: const TextStyle(fontWeight: FontWeight.w600)), subtitle: Text('@${entry.$2.toLowerCase().replaceAll(' ', '_')}'), trailing: Text(entry.$1 < 2 ? 'Admin' : entry.$1 < 5 ? 'Online' : 'Offline', style: TextStyle(fontSize: 11, color: entry.$1 < 2 ? authPurple : entry.$1 < 5 ? Colors.green : authMuted)))),
